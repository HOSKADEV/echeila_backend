...
@section('title', __('trip.car_rescues'))

@section('content')
  <!-- Header Section -->
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1">{{ __('trip.car_rescues') }}</h4>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
          <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('app.dashboard') }}</a></li>
          <li class="breadcrumb-item"><a href="{{ url('admin/trips/all') }}">{{ __('trip.trips') }}</a></li>
          <li class="breadcrumb-item active">{{ __('trip.car_rescues') }}</li>
        </ol>
      </nav>
    </div>
  </div>
