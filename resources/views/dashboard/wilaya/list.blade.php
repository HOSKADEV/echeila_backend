...
@section('title', __('app.wilayas'))

@section('content')
  <!-- Header Section -->
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1">{{ __('app.wilayas') }}</h4>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
          <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('app.dashboard') }}</a></li>
          <li class="breadcrumb-item active">{{ __('app.wilayas') }}</li>
        </ol>
      </nav>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="card-datatable table-responsive">
      <div class="dataTables_wrapper dt-bootstrap5 no-footer">
          ...
          <div class="col-md-10">
            <div
              ...
              <div class="dt-buttons btn-group flex-wrap">
                ...
                @permission(\App\Support\Enum\Permissions::WILAYA_CREATE)
                <a href="{{ route('wilayas.create') }}" class="text-white text-decoration-none">
                  <button type="button" class="btn btn-primary">
                    <span class="tf-icons bx bx-plus"></span> @lang('app.add-new-wilaya')
                  </button>
                </a>
                @endpermission
              </div>
        ...
        <x-table.datatable :columns="$columns" table-id="laravel_datatable" translation-prefix="wilaya" />
  ...
  <x-modal.confirmation
    id="delete-modal"
    title="{{ __('app.delete') }}"
    action="{{ route('wilayas.destroy', ':id') }}"
    method="DELETE"
    inputs='
