...
@section('title', __('app.edit-seat-price'))

@section('content')
  <!-- Header Section -->
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1">{{ __('app.edit-seat-price') }}</h4>
  ...
  <form action="{{ route('seat-prices.update', $seatPrice->id) }}" method="POST" id="seat-price-form">
    @csrf
    @method('PATCH')
    <div class="row">
      <div class="col-xl">
          ...
          <div class="row card-body">
            <div class="mb-3 col-md-6">
              ...
              <select 
                ...
                @foreach($wilayas as $wilaya)
                  <option value="{{ $wilaya->id }}" {{ old('starting_wilaya_id', $seatPrice->starting_wilaya_id) == $wilaya->id ? 'selected' : '' }}>
            ...
            <div class="mb-3 col-md-6">
              ...
              <select 
                ...
                <option value="">{{ __('app.select_option') }}</option>
                @foreach($wilayas as $wilaya)
                  <option value="{{ $wilaya->id }}" {{ old('arrival_wilaya_id', $seatPrice->arrival_wilaya_id) == $wilaya->id ? 'selected' : '' }}>
                    {{ $wilaya->translate('name', app()->getLocale()) }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="mb-3 col-md-12">
              ...
              <div class="input-group">
                <input 
                  ...
                  value="{{ old('default_seat_price', $seatPrice->default_seat_price ?? '') }}" 
                  step="0.01"
                  min="0"
                  max="999999.99"
                  required>
