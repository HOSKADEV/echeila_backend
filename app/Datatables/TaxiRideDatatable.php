...
class TaxiRideDatatable
{
        ...
        try {
            return datatables($this->query($request))
                ->addColumn('actions', function ($model) {
                    return $this
                        ->show(route('trips.show', $model->id), Auth::user()->hasPermissionTo(Permissions::TAXI_RIDE_SHOW))
                        ->makeLabelledIcons();
                })
                ...
                ->addColumn('from_to', function ($model) {
                ...
                ->addColumn('type', function ($model) {
                    $details = $model->detailable;
                    $statusName = RideType::get_name($details->ride_type);
                    $color = RideType::get_color($details->ride_type);
    ...
    public function query($request)
    {
        ...
        // Filter by ride type - Use whereHasMorph
        if ($request->ride_type_filter) {
            $query->whereHasMorph('detailable', [\App\Models\TaxiRideDetail::class], function ($q) use ($request) {
                $q->where('ride_type', $request->ride_type_filter);
            });
        }
